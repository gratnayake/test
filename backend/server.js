...
app.post('/api/kubernetes/snapshot-pod-monitoring/retake-snapshot', async (req, res) => {
  try {
    ...
    if (success) {
      ...
      res.status(500).json({
        success: false,
        error: 'Failed to retake snapshot'
      });
    }
  } catch (error) {
    console.error('❌ Retake snapshot error:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
app.post('/api/kubernetes/snapshot-pod-monitoring/clear-snapshot', (req, res) => {
  try {
    ...
    if (cleared) {
      ...
      res.status(500).json({
        success: false,
        error: 'Failed to clear snapshot'
      });
    }
  } catch (error) {
...
app.post('/api/kubernetes/snapshot-pod-monitoring/clear-down-pods', (req, res) => {
  try {
    ...
    if (cleared) {
      ...
      res.status(500).json({
        success: false,
        error: 'Failed to clear down pods file'
      });
    }
  } catch (error) {
    console.error('❌ Clear down pods error:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
